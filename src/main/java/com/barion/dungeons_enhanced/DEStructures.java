...
public class DEStructures {
    ...
    private static  <S extends GelConfigStructure<NoneFeatureConfiguration>> StructureRegistrar<NoneFeatureConfiguration, S> register(String registryName, boolean adaptNoise, Supplier<S> structure, StructurePieceType piece, GenerationStep.Decoration decoration){
        if(adaptNoise) {
            return StructureRegistrar.builder(location(registryName), structure).pushConfigured(NoneFeatureConfiguration.INSTANCE).biomes(structure.get().getConfig().getConfigured()).adaptNoise().popConfigured().addPiece(piece).generationStep(decoration).build();
        }
        return StructureRegistrar.builder(location(registryName), structure).pushConfigured(NoneFeatureConfiguration.INSTANCE).biomes(structure.get().getConfig().getConfigured()).popConfigured().addPiece(piece).generationStep(decoration).build();
    }
    private static  <S extends GelConfigStructure<NoneFeatureConfiguration>> StructureRegistrar<NoneFeatureConfiguration, S> register(String registryName, boolean adaptNoise, Supplier<S> structure, StructurePieceType piece, GenerationStep.Decoration decoration, MobCategory spawnCategory, MobSpawnSettings.SpawnerData... spawns){
        if(adaptNoise) {
            return StructureRegistrar.builder(location(registryName), structure).pushConfigured(NoneFeatureConfiguration.INSTANCE).biomes(structure.get().getConfig().getConfigured()).spawns(spawnCategory, StructureSpawnOverride.BoundingBoxType.STRUCTURE, spawns).adaptNoise().popConfigured().addPiece(piece).generationStep(decoration).build();
        }
        return StructureRegistrar.builder(location(registryName), structure).pushConfigured(NoneFeatureConfiguration.INSTANCE).biomes(structure.get().getConfig().getConfigured()).popConfigured().addPiece(piece).generationStep(decoration).build();
    }
    ...
    private static <C extends JigsawConfiguration, S extends GelConfigJigsawStructure<C>> StructureRegistrar<C, S> registerJigsaw(String registryName, boolean adaptNoise, Supplier<S> structure, C configuration, StructurePieceType piece, GenerationStep.Decoration generationStep){
        if(adaptNoise){
            return StructureRegistrar.builder(location(registryName), structure).pushConfigured(configuration).biomes(structure.get().getConfig().getConfigured()).adaptNoise().popConfigured().addPiece(piece).generationStep(generationStep).build();
        }
        return StructureRegistrar.builder(location(registryName), structure).pushConfigured(configuration).biomes(structure.get().getConfig().getConfigured()).popConfigured().addPiece(piece).generationStep(generationStep).build();
    }
    private static <C extends JigsawConfiguration, S extends GelConfigJigsawStructure<C>> StructureRegistrar<C, S> registerJigsaw(String registryName, boolean adaptNoise, Supplier<S> structure, C configuration, StructurePieceType piece, GenerationStep.Decoration generationStep, MobSpawnSettings.SpawnerData... spawns){
        if(adaptNoise){
            return StructureRegistrar.builder(location(registryName), structure).pushConfigured(configuration).biomes(structure.get().getConfig().getConfigured()).spawns(MobCategory.MONSTER, StructureSpawnOverride.BoundingBoxType.STRUCTURE, spawns).adaptNoise().popConfigured().addPiece(piece).generationStep(generationStep).build();
        }
        return StructureRegistrar.builder(location(registryName), structure).pushConfigured(configuration).biomes(structure.get().getConfig().getConfigured()).popConfigured().addPiece(piece).generationStep(generationStep).build();
    }
    ...
    private static StructureRegistrar<JigsawConfiguration, DECellarStructure> registerCellarStructure(String registryName, Supplier<DECellarStructure> structure, Holder<StructureTemplatePool> pool){
        return StructureRegistrar.builder(location(registryName), structure).pushConfigured(new JigsawConfiguration(pool, 1)).biomes(structure.get().getConfig().getConfigured()).adaptNoise().popConfigured().addPiece(DECellarStructure.Piece::new).generationStep(GenerationStep.Decoration.SURFACE_STRUCTURES).build();
    }
}